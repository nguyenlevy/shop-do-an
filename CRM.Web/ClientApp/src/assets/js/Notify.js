(function (plugin) {
	...
	var containers = {},
		...
		notify = function (text, className) {
			...
			if ($.type(text) == 'string') {
				options = { message: text, status: className };
			}

			if (arguments[1]) {
				options = $.extend(options, $.type(arguments[1]) == 'string' ? { status: arguments[1].className } : arguments[1]);
			}
	...
	var Message = function (options) {
		...
		this.options.status = options.className;
		this.options.timeout = options.autoHideDelay;
