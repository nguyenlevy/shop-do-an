...
export class ListSuppliersComponent implements OnInit {
  ...
  constructor(private dialogService: DialogService,
    ...
    this.langDisplay = this.translateService.get(['TAB_TITLE', 'SUPPLIER', 'BUTTONS', 'NOTIFICATION']).then((res: any) => {
      if (res) {
        titleService.setTitle(res.TAB_TITLE.SUPPLIER);
        this.langDisplay = res.SUPPLIER;
  ...
  initColumns() {
    this.cols = [
      ...
      {
        field: 'customerCode',
        header: this.langDisplay && this.langDisplay.CUSTOMER_CODE ? this.langDisplay.CUSTOMER_CODE : 'Mã NCC',
        width: '15%',
        sort: true,
      ...
      {
        field: 'name',
        header: this.langDisplay && this.langDisplay.NAME ? this.langDisplay.NAME : 'Tên NCC',
        width: '15%',
        sort: true,
        filterMatchMode: 'contains'
      },
  ...
  showPopup(title: string, data: any, isUpdate: boolean = false) {
    const ref = this.dialogService.open(FormSupplierComponent, {
      header: title,
      width: '800px',
      contentStyle: {
  ...
  delete(id) {
    this.confirmationService.confirm({
      message: this.langDisplayMessage && this.langDisplayMessage.XAC_NHAN ?
        this.langDisplayMessage.XAC_NHAN : 'Bạn chắc chắn muốn xóa dòng này?',
      header: this.langDisplayMessage && this.langDisplayMessage.TIEU_DE_XAC_NHAN ?
        this.langDisplayMessage.TIEU_DE_XAC_NHAN : 'Xác nhận',
  ...
  getExportData() {
    return this.mainItems.map(
      (json) => {
        return new SupplierModel().exportData(json);
      }
    );
  }
